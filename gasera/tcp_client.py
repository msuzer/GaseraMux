...
from system.log_utils import info, warn, error, verbose, debug
...
class GaseraTCPClient:
    ...
    def _recv_until_stx_etx(self, overall_timeout: Optional[float] = None) -> Optional[str]:
        ...
        while time.monotonic() < deadline:
            ...
            buf += chunk
            debug( f"recv {len(chunk)}B: {_hexsample(chunk)} (buf={len(buf)}B)", verbose=self.verbose)
            ...
            if last_stx != -1:
                if last_stx > 0:
                    ...
                    if junk:
                        debug( f"Discarding {len(junk)}B before STX: {_hexsample(junk)}", verbose=self.verbose)
                ...
                try:
                    ...
                    debug( f"STX..ETX frame {len(frame)}B found (payload {len(payload)}B): '{pretty}'", verbose=self.verbose)
